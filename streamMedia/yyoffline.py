...
# Get the base stream
def getBaseStream(queryUrl=None, starttime=None, endtime=None):
...
def excute_send_expect(child=None, commands=None, exp_prompt='root@', type=None, starttime=None, endtime=None, streams=None, queryUrl=None):
    ...
    command_basic = [
      '''curl -i -d "[{\\"method\\":\\"StreamBaseSearch\\",\\"version\\":\\"2\\",\\"domains\\":[\\"newissupper\\"], \\"startTime\\":\\"%s\\", \\"endTime\\":\\"%s\\"}]" "%s"''' % (starttime, endtime, queryUrl),
    ]

    command_status = [
      '''curl -i -d "[{\\"interval\\":\\"1\\",\\"method\\":\\"StreamStatusSearch\\",\\"version\\":\\"2\\",\\"type\\":0, \\"protocols\\":[1,2,3,4,5],\\"urls\\":[\\111\\"], \\"startTime\\":\\"%s\\", \\"endTime\\":\\"%s\\"}]" "%s"''' % (starttime, endtime, queryUrl),
    ]

    command_domain = [
      '''curl -i -d "[{\\"interval\\":\\"1\\",\\"method\\":\\"DomainStatusSearch\\",\\"version\\":\\"2\\",\\"type\\":0, \\"protocols\\":[1,2,3,4,5],\\"domains\\":[\\"newissupper\\"], \\"startTime\\":\\"%s\\", \\"endTime\\":\\"%s\\"}]" "%s"''' % (starttime, endtime, queryUrl)
    ]
    ...
    for stream in eval(streams):
        child.sendline ('''curl -i -d "[{\\"interval\\":\\"1\\",\\"method\\":\\"StreamStatusSearch\\",\\"version\\":\\"2\\",\\"type\\":0, \\"protocols\\":[1,2,3,4,5],\\"urls\\":[\\"%s\\"], \\"startTime\\":\\"%s\\", \\"endTime\\":\\"%s\\"}]" "%s"''' % (stream,starttime, endtime,queryUrl))
        i = child.expect(['(\[\[\{.*)root@','(\[\[\{.*\}\]\])','\[\{\}\]', '500 Internal Server Error'])
        if i == 0 or i == 1:
            result = child.match.group(1)
            result = re.sub(r"(?m)(\r\n\[)", "", result)
...
queryUrl = "http://192.168.32.220:8081/qk_dw?cchost_tag=newisscenter"
...
if len(sys.argv) != 1:
    ...
    excute_send_expect(queryUrl=queryUrl, child=child, starttime=starttime, endtime=endtime, streams=streams)
    ...
    streams = getBaseStream(queryUrl, starttime,endtime)
    if not re.search('rtmp', streams):
        print 'No streams!'
        sys.exit(0)
